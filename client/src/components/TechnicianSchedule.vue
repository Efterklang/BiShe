...
import { getSchedules, batchSetSchedule } from '../api/schedules';
...
const currentLabel = computed(() => {
  ...
  return currentDate.value.toLocaleDateString('zh-CN', { year: 'numeric', month: 'long', day: 'numeric', weekday: 'short' });
});

const activeTechnicians = computed(() => {
  if (selectedTechIds.value.length === 0) return technicians.value;
  return technicians.value.filter(t => selectedTechIds.value.includes(t.id));
});
...
const fetchSchedules = async () => {
  ...
  try {
    let start, end;
    if (viewMode.value === 'month') {
      start = formatDate(new Date(currentYear.value, currentMonth.value, 1));
      end = formatDate(new Date(currentYear.value, currentMonth.value + 1, 0));
    } else {
    ...
    const params = { start_date: start, end_date: end, _t: new Date().getTime() };
...
const fetchAppointments = async (dateStr) => {
  try {
    ...
    appointments.value = (allAppts || []).filter(app => app.start_time.startsWith(dateStr));
  } catch (e) {
...
const getDayStatus = (dateStr) => {
  const daySchedules = schedules.value.filter(s => s.date.startsWith(dateStr));
  ...
  return { total: totalTechs, leave: leaveCount, work: totalTechs - leaveCount };
};

const getTechDaySchedule = (techId) => {
  const dateStr = formatDate(currentDate.value);
  const s = schedules.value.find(s => s.date.startsWith(dateStr) && s.tech_id === techId);
  return s ? s.is_available : true; // Default available
};

const getTechAppointments = (techId) => {
  return appointments.value.filter(a => a.tech_id === techId).map(a => {
  ...
  <div class="bg-base-100 rounded-xl border border-base-300 shadow-sm overflow-hidden">
    ...
    <div class="p-4 border-b border-base-200 flex flex-col md:flex-row justify-between items-center gap-4">
      ...
      <div class="flex items-center gap-2">
        <div class="join">
          ...
          <button @click="goToToday" class="btn btn-sm join-item">今天</button>
          <button @click="changeDate(1)" class="btn btn-sm join-item">
            <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke-width="1.5" stroke="currentColor" class="w-4 h-4">
        ...
        <span class="text-lg font-bold ml-2 min-w-[140px]">{{ currentLabel }}</span>
      </div>
      ...
      <div class="flex items-center gap-3 flex-wrap justify-end">
        ...
        <div class="dropdown dropdown-end">
          ...
          <div tabindex="0" class="dropdown-content z-[1] menu p-2 shadow bg-base-100 rounded-box w-52 max-h-64 overflow-y-auto border border-base-200">
            <li v-for="tech in technicians" :key="tech.id">
              <label class="label cursor-pointer justify-start gap-2">
                <input type="checkbox" class="checkbox checkbox-xs checkbox-primary" :value="tech.id" v-model="selectedTechIds" />
    ...
    <div class="relative min-h-[500px]">
      ...
      <div v-if="viewMode === 'month'" class="p-4">
        ...
        <div class="grid grid-cols-7 gap-2">
          <div
            ...
            :class="[
              day.date ? 'hover:border-primary/50 bg-base-100' : 'border-transparent bg-transparent pointer-events-none',
              day.isToday ? 'ring-2 ring-primary ring-offset-2' : 'border-base-200',
              day.isSelected ? 'bg-primary/10 border-primary' : ''
            ]"
            @click="handleDayClick(day)"
          >
            <div v-if="day.date">
              <span class="text-sm font-medium" :class="day.isToday ? 'text-primary' : ''">{{ day.date.getDate() }}</span>

              <!-- Status Dots -->
              <div class="mt-2 space-y-1">
                <div v-if="getDayStatus(day.dateStr).leave > 0" class="flex items-center gap-1 text-xs text-error">
                  <span class="w-2 h-2 rounded-full bg-error"></span>
                  <span>{{ getDayStatus(day.dateStr).leave }} 休</span>
                </div>
                <div v-if="getDayStatus(day.dateStr).work > 0" class="flex items-center gap-1 text-xs text-success">
                  <span class="w-2 h-2 rounded-full bg-success"></span>
                  <span>{{ getDayStatus(day.dateStr).work }} 班</span>
                </div>
              </div>
      ...
      <div v-else class="flex flex-col h-full overflow-x-auto">
        <!-- Header Time Scale -->
        <div class="flex border-b border-base-200 bg-base-200/30 min-w-[800px]">
          <div class="w-32 flex-shrink-0 p-3 font-semibold text-sm border-r border-base-200 bg-base-100 sticky left-0 z-20">技师</div>
        ...
        <div class="flex-1 overflow-y-auto min-w-[800px]">
          ...
          >
            <!-- Tech Name -->
            <div class="w-32 flex-shrink-0 p-3 flex items-center gap-2 border-r border-base-200 bg-base-100 sticky left-0 z-10">
              <div class="w-8 h-8 rounded-full bg-base-300 flex items-center justify-center text-xs font-bold">
                {{ tech.name.charAt(0) }}
              </div>
              <div class="flex flex-col">
                <span class="text-sm font-medium truncate w-16">{{ tech.name }}</span>
                <span
                  class="text-[10px] px-1.5 py-0.5 rounded-full w-fit"
                  :class="getTechDaySchedule(tech.id) ? 'bg-success/10 text-success' : 'bg-error/10 text-error'"
                >
                  {{ getTechDaySchedule(tech.id) ? '在岗' : '休息' }}
                </span>
              </div>
            </div>
            ...
            <div class="flex-1 relative bg-base-100">
              ...
              <div class="absolute inset-0 flex pointer-events-none">
                <div v-for="hour in timeSlots" :key="hour" class="flex-1 border-l border-base-200 border-dashed"></div>
              </div>
              ...
              >
                <span class="text-xs text-base-content/40 font-medium tracking-widest">RESTING</span>
                <div class="absolute inset-0" style="background-image: repeating-linear-gradient(45deg, transparent, transparent 10px, rgba(0,0,0,0.03) 10px, rgba(0,0,0,0.03) 20px);"></div>
              </div>
              ...
              <div v-else class="absolute inset-0 top-2 bottom-2">
                ...
                >
                  <div class="font-bold truncate">{{ appt.member?.name || 'Guest' }}</div>
                  <div class="opacity-80 truncate scale-90 origin-top-left">{{ appt.service_item?.name }}</div>
                </div>
              </div>
            </div>
          </div>
