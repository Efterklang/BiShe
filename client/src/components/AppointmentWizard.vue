...
const selectedService = computed(() => {
    // Use local services list first as it contains reliable price data
    const localService = services.value.find(
        (s) => s.id === Number(formData.value.service_id),
    );

    if (localService) return localService;

    // Fallback to API response from getAvailableTechnicians
...
const discountRate = computed(() => {
    ...
    switch (selectedMember.value.level) {
...
const discountedPrice = computed(() => {
    ...
    const price = selectedService.value.price * discountRate.value;
                    ...
                    <div class="form-control">
                        ...
                        <select v-model="formData.service_id" class="select select-bordered w-full" required>
                            ...
                            <option v-for="s in services" :key="s.id" :value="s.id"
                                :disabled="s.available_tech_count === 0">
                                {{ s.name }} - ¥{{ s.price }} ({{ s.duration }}分钟)
                                {{ s.available_tech_count === 0 ? '(当前项目无可用技师)' : '' }}
                            </option>
                        </select>
                    </div>
                    ...
                    <div class="form-control" v-if="formData.service_id && selectedDate">
                        ...
                        <div v-else class="grid grid-cols-4 gap-2 max-h-60 overflow-y-auto p-1">
                            <button v-for="slot in timeSlots" :key="slot.time" @click="selectTimeSlot(slot)"
                                ...
                                {{ slot.time }}
                                <span v-if="slot.status === 'available'" class="text-[10px] ml-1">({{
                                ...
                                <div
                                    ...
                                    <div class="card-body p-4">
                                        <div class="flex items-center gap-3">
                                            <div
                                                class="w-12 h-12 rounded-full bg-success/10 flex items-center justify-center overflow-hidden text-lg font-bold text-success">
                                                <img v-if="tech.avatar_url" :src="tech.avatar_url" :alt="tech.name"
                                                    class="w-full h-full object-cover" />
                                                <span v-else>{{ tech.name.charAt(0) }}</span>
                                            </div>
                                            <div class="flex-1">
                    ...
                    <div class="bg-base-200 rounded-lg p-4 space-y-3">
                        ...
                        <div class="flex justify-between items-center">
                            ...
                            <div class="text-right">
                                <span v-if="discountRate < 1" class="text-base-content/60 text-sm line-through mr-2">¥{{
                                    selectedService?.price }}</span>
                                <span class="font-medium" :class="{ 'text-success': discountRate < 1 }">¥{{
