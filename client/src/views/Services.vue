...
const openCreateModal = () => {
  ...
  formData.value = { name: '', duration: 60, price: 0, is_active: true };
...
<template>
  <div class="max-w-7xl mx-auto">
    ...
    <div class="flex flex-col md:flex-row md:items-center justify-between mb-10 gap-4">
      <div>
        <h1 class="text-3xl font-bold tracking-tight text-base-content">服务项目</h1>
    ...
    <!-- Empty State -->
    <div v-else-if="services.length === 0" class="flex flex-col items-center justify-center py-20 border border-dashed border-base-300 rounded-xl bg-base-200/50">
        ...
        <div class="mt-auto pt-4 border-t border-base-200 flex items-center justify-between">
          <div class="flex items-center gap-2">
            <span class="relative flex h-2.5 w-2.5">
              <span v-if="service.is_active || service.IsActive" class="animate-ping absolute inline-flex h-full w-full rounded-full bg-success opacity-75"></span>
              <span class="relative inline-flex rounded-full h-2.5 w-2.5" :class="(service.is_active || service.IsActive) ? 'bg-success' : 'bg-base-300'"></span>
            </span>
            <span class="text-xs font-medium" :class="(service.is_active || service.IsActive) ? 'text-success' : 'text-base-content/60'">
              {{ (service.is_active || service.IsActive) ? '上架中' : '已下架' }}
            </span>
          </div>

          <div class="flex gap-2">
             <button @click="handleEdit(service)" class="btn btn-ghost btn-sm btn-square text-base-content/60 hover:text-base-content">
                <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke-width="1.5" stroke="currentColor" class="w-4 h-4">
                  <path stroke-linecap="round" stroke-linejoin="round" d="M16.862 4.487l1.687-1.688a1.875 1.875 0 112.652 2.652L10.582 16.07a4.5 4.5 0 01-1.897 1.13L6 18l.8-2.685a4.5 4.5 0 011.13-1.897l8.932-8.931zm0 0L19.5 7.125M18 14v4.75A2.25 2.25 0 0115.75 21H5.25A2.25 2.25 0 013 18.75V8.25A2.25 2.25 0 015.25 6H10" />
                </svg>
             </button>
             <button @click="handleDelete(service.id)" class="btn btn-ghost btn-sm btn-square text-base-content/60 hover:text-error hover:bg-error/10">
                <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke-width="1.5" stroke="currentColor" class="w-4 h-4">
    ...
    <dialog class="modal" :class="{ 'modal-open': showModal }">
      <div class="modal-box bg-base-100 border border-base-300 shadow-2xl rounded-xl p-0 overflow-hidden max-w-md">
        ...
        <div class="px-6 py-4 border-b border-base-200 flex justify-between items-center bg-base-200/50">
          <h3 class="font-semibold text-lg text-base-content">{{ editingId ? '编辑服务项目' : '新增服务项目' }}</h3>
          ...
          <form @submit.prevent="handleSubmit" class="space-y-5">
              ...
              <div>
                ...
                <div class="relative">
                  ...
                  <input
                    ...
                    min="0"
                    class="input input-bordered w-full pl-7 bg-base-100"
                    required
                  />
                </div>
              </div>
            ...
            <div class="pt-2">
              ...
              >
                ...
                {{ submitting ? '保存中...' : (editingId ? '确认修改' : '确认新增') }}
              </button>
            </div>
          </form>
        </div>
