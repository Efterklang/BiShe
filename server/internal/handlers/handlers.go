...
func GetDashboardStats(c *gin.Context) {
	...
	if yesterdayRevenue > 0 {
		revenueGrowth = util.RoundMoney(((dailyRevenue - yesterdayRevenue) / yesterdayRevenue) * 100)
	}
...
func CreateAppointment(c *gin.Context) {
	...
	// Get Member details for discount
	var member models.Member
	if err := db.DB.First(&member, req.MemberID).Error; err != nil {
		c.JSON(http.StatusBadRequest, response.Error(http.StatusBadRequest, "Member not found", nil))
		return
	}

	// Calculate Discount based on Member Level
	discountRate := config.GlobalMemberDiscount.Basic
	switch member.Level {
	case "platinum":
		discountRate = config.GlobalMemberDiscount.Platinum
	case "gold":
		discountRate = config.GlobalMemberDiscount.Gold
	case "silver":
		discountRate = config.GlobalMemberDiscount.Silver
	}
	// Use CalculateRate to handle multiplication and rounding
	actualPrice := util.CalculateRate(service.Price, discountRate)
...
func CompleteAppointment(c *gin.Context) {
	...
	member.Level = util.CalculateMemberLevel(member.YearlyTotalConsumption)
	...
	if member.ReferrerID != nil {
		// 使用 utils.CalculateRate 计算佣金，自动处理精度
...
func ListServiceItems(c *gin.Context) {
	...
	// Calculate AvailableTechCount for each service
	var technicians []models.Technician
	// Get all active technicians (not deleted)
	if err := db.DB.Find(&technicians).Error; err != nil {
		// Log error but proceed
		fmt.Printf("Failed to fetch technicians for count: %v\n", err)
	}

	type ServiceResponse struct {
		models.ServiceProduct
		AvailableTechCount int `json:"available_tech_count"`
	}

	var responseItems []ServiceResponse

	for _, item := range items {
		count := 0
		for _, tech := range technicians {
			// Check if tech has this skill
			if len(tech.Skills) > 0 {
				var skills []uint
				// tech.Skills is []byte (json.RawMessage/datatypes.JSON)
				if err := json.Unmarshal(tech.Skills, &skills); err == nil {
		...
		responseItems = append(responseItems, ServiceResponse{
			ServiceProduct:     item,
			AvailableTechCount: count,
		})
	}

	c.JSON(http.StatusOK, response.Success(responseItems, ""))
}
